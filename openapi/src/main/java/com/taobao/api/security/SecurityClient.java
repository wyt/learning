...
public class SecurityClient implements SecurityConstants {
     ...
     *            线程池最大队列
     */
    public SecurityClient(TaobaoClient taobaoClient, String randomNum, int corePoolSize, int maxPoolSize,
            int maxQueue) {
        secretCore = new SecurityCore(taobaoClient, randomNum, corePoolSize, maxPoolSize, maxQueue);
    }
    ...
    private String encrypt(String data, String type, String session, Long version) throws SecretException {
        ...
        try {
            ...
            // 公共秘钥版本号用负数区分
            if (session == null) {
                secretContext.setSecretVersion(-secretContext.getSecretVersion());
            }
